...
const Materi = () => {
  ...
  const getData = async () => {
    try {
      setIsLoading(prevState => [...prevState, 'fetch-materials']);
      const materials = await axios.get(`/materials`)
      setMaterials(materials.data.data)
    } catch (error) {
      toast.error(error.response?.data?.message || error.message)
    } finally {
      setIsLoading(prevState => prevState.filter(item => item !== 'fetch-materials'));
    }
  }
          ...
          <Card>
            <div className="overflow-x-auto">
              <table className="table">
                ...
                <tbody>
                  {getIsLoading.includes('fetch-materials') ? (
                    <tr>
