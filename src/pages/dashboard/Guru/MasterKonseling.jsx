...
const MasterKonseling = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`/master/counselings`)
                  ...
                  {getData.map((data, idx) => (
                    <tr key={idx} className="hover">
                      <td className="align-top">
                        <span className={`px-2 py-1 rounded ${data.status === 'PENDING' ? 'bg-gray-300' : data.status === 'BERLANGSUNG' ? 'bg-info' : 'bg-success'}`}>
                          {data.status}
                        </span>
                      </td>
                      <td className="align-top">{data.subject}</td>
                      <td className="align-top">{data.content}</td>
...
export default MasterKonseling